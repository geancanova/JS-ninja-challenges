(function () {
  ...
  */
  const cpfRegx = /(\d{3})(\d{3})(\d{3})(\d{2})/g;

  const formatCPF = (cpf, group1, group2, group3, group4) => {
    return `${group1}.${group2}.${group3}-${group4}`;
  };

  console.log("\nFormatando CPFs corretamente:");
  console.log(cleanCPF("049-214 3421-1").replace(cpfRegx, formatCPF));
  console.log(cleanCPF("210.458.522-05").replace(cpfRegx, formatCPF));
  console.log(cleanCPF("735 500 794 - 22").replace(cpfRegx, formatCPF));
  console.log(cleanCPF("101.123-131x32").replace(cpfRegx, formatCPF));
  ...
  */
  const htmlString =
    "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";
  const htmlRegx = /<(\w*)>([^<]+)\<\/\w+>/g;

  console.log("\nFazer replace dos textos das tags:");
  console.log(
    htmlString.replace(htmlRegx, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n')
  );
})();
